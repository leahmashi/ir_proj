...
class MultiFileWriter:
    ...
    def __init__(self, base_dir, name, bucket_name, index_num):
        self._base_dir = Path(base_dir)
        self._name = name
        self.indexNum = index_num
        self._file_gen = (open(self._base_dir / f'{name + index_num}_{i:03}.bin', 'wb')
...
class MultiFileReader:
    ...
    def read(self, locs, n_bytes, path):
        b = []
...
class InvertedIndex:
    ...
    def posting_lists_iter(self):
        ...
        with closing(MultiFileReader()) as reader:
            for w, locs in self.posting_locs.items():
                b = reader.read(locs[0], self.df[w] * TUPLE_SIZE, "./postings_gcp")
                posting_list = []
    ...
    @staticmethod
    def write_a_posting_list(b_w_pl, bucket_name, num):
        posting_locs = defaultdict(list)
        bucket_id, list_w_pl = b_w_pl

        with closing(MultiFileWriter(".", bucket_id, bucket_name, num)) as writer:
